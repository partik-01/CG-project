/* C program to show solar system using computer graphics */
#include<conio.h>
#include<stdio.h>
#include <graphics.h>
#include <math.h>
...
void main()
{
    int gd = DETECT, gm, i, midx, midy, xrad[9], yrad[9], x[9][70], y[9][70], pos[9], planet[9], tmp;
    initgraph(&gd, &gm,"C:\\TURBOC3\\BGI");
    midx = getmaxx() / 2;
    midy = getmaxy() / 2;
    // Manipulating radius of all the nine planets
    planet[0] = 8;
    for (i = 1; i < 9; i++)
    ...
    while (1)
    {
        if (kbhit())
            break;
        for (i = 0; i < 9; i++)    //drawing orbits only
        {
            setcolor(CYAN);
            ellipse(midx, midy, 0, 360, xrad[i], yrad[i]);
        }
        // Sun at the centre of solar system
        setcolor(YELLOW);
